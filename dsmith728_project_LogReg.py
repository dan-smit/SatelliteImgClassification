...
#The best parameters turn out to be isomap n_neighbors = 10 and logreg solver = 'liblinear'
...
param_grid = {
    ...
    'logreg__solver': ['sag', 'saga', 'liblinear'] 
}
...
logreg_model = LogisticRegression(max_iter=200, solver='liblinear').fit(X_train_isomap, y_train)
logreg_predictions = logreg_model.predict(X_test_isomap)
logreg_cm = confusion_matrix(y_test, logreg_predictions)
print(classification_report(y_test, logreg_predictions, zero_division=0.0))
logreg_cm_disp = ConfusionMatrixDisplay(logreg_cm)
logreg_cm_disp.plot(colorbar = False)
